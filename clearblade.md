...
This class allows for the execution of async ClearBlade query functions and returns promises. The `init` function is not required. The functions are the same as the [ClearBlade Query Class](#query)

To instantiate the async query class just call:

~~~javascript
	var async = ClearBladeAsync.Query("Activity")
~~~


Class: ClearBladeAsync.Collections(options)
...
~~~javascript
	var async = ClearBladeAsync.Collection(options)
~~~
## ClearBladeAsync.newCollection(name)

Creates a new ClearBlade collection asynchronously.

* @param {string} name
* @returns {Promise}

~~~javascript
 ClearBladeAsync.Collection("<COLLECTION_NAME>")
~~~
...
~~~javascript
// Use 'collectionName' for collection name or 'collection' for collection ID  
	var async = ClearBladeAsync.Collection({collectionName:"<COLLECTION_NAME>");
	async.fetch()
		.then()
		.then(resp.success)
...
~~~javascript
	var async = ClearBladeAsync.Collection("<COLLECTION_NAME>")
...
~~~javascript
	var async = ClearBladeAsync.Collection("Name")
	var newItem = {
        column: '<COL_DATA>',
	}
